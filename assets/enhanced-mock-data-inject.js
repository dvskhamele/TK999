// Enhanced mock data for the betting application - to be injected into the main bundle
...
// Enhanced mock API functions - to be used with the main bundle
window.enhancedMockAPI = {
  getUser: () => {
    return new Promise((resolve) => {
      setTimeout(() => resolve(window.enhancedMockData.user), 200);
    });
  },
  
  getDashboardStats: () => {
    return new Promise((resolve) => {
      setTimeout(() => resolve(window.enhancedMockData.dashboardStats), 200);
    });
  },
  ...
  getMatches: (filters = {}) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        let matches = [...window.enhancedMockData.matches];
  ...
  placeBet: (betData) => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        ...
        if (!userData.id) {
          reject({ success: false, message: 'Please log in to place bets' });
          return;
        }
  ...
  toggleFavorite: (matchId) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        const match = window.enhancedMockData.matches.find(m => m.id === matchId);
        if (match) {
  ...
  getUserBets: () => {
    return new Promise((resolve) => {
      setTimeout(() => resolve(window.enhancedMockData.bets), 200);
    });
  }
};